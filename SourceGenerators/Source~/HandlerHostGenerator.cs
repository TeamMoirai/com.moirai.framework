...
{
    /// <summary>
    /// 为标记了 [HandlerHost(typeof(THandler))] 的 static partial class 生成：
    /// s_Handler 字段、Handler 属性（线程安全 get/set）。
    /// <para>s_Handler 为 private，partial 同类可直接访问。</para>
    /// <para>当 CreateDefaultHandler 方法存在时额外生成 s_DefaultFactory 字段与懒加载路径；
    /// 缺失时 Handler.get 直接抛 InvalidOperationException，不引用 CreateDefaultHandler，
    /// 由 HandlerHostAnalyzer (MIRAI001) 报告诊断。</para>
    /// </summary>
    [Generator]
    public class HandlerHostGenerator : IIncrementalGenerator
    {
        ...
        private static string GenerateCode(in HandlerHostInfo info)
        {
            ...
            sb.AppendLine("{");
            sb.AppendLine($"    static partial class {info.ClassName}");
            sb.AppendLine("    {");
        ...
        private readonly struct HandlerHostInfo
        {
            ...
            public HandlerHostInfo(string className, string @namespace, string handlerTypeName, bool hasCreateDefaultHandler)
            {
                ClassName = className;
                Namespace = @namespace;
