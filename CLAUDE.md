...
- `CHANGELOG.md` **只有 `[Unreleased]` 一段**：已发布的内容不留在文件里。发版由 `build-release` 工作流收尾——该段切成 GitHub Release 的 notes 并 Publish，之后自动开一个清空该段的 PR。版本号与 `package.json` 的 `version` 由同一条自动化写入，不在手上改。
