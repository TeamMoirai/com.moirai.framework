...
namespace Moirai.Atropos.Timer
{
    ...
    /// <para><b>它不是 NUnit 用例，不参与测试套件</b>——本类型没有 <c>[Test]</c>/<c>[UnityTest]</c>，
    /// Test Runner 不会执行它，也不需要 <c>[Explicit]</c>（那是给 NUnit 基准用的）。自动套件里的基准见
    /// 各模块目录下的 <c>XxxBenchmark</c>（一律 <c>[Explicit]</c>）。</para>
