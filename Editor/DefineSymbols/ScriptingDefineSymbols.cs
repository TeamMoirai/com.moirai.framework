...
namespace Moirai.Atropos.Editor
{
    ...
    {
        private static IEnumerable<BuildTargetGroup> PresentBuildTargetGroups =>
            from target in (BuildTarget[])Enum.GetValues(typeof(BuildTarget))
            let @group = BuildPipeline.GetBuildTargetGroup(target)
            where BuildPipeline.IsBuildTargetSupported(@group, target)
            select @group;
        ...
        public static void RemoveScriptingDefineSymbol(BuildTargetGroup buildTargetGroup, string scriptingDefineSymbol)
        {
            ...
            List<string> scriptingDefineSymbols = new List<string>(GetScriptingDefineSymbols(buildTargetGroup));
            for (int i = scriptingDefineSymbols.Count - 1; i >= 0; i--)
            {
                if (scriptingDefineSymbols[i] == scriptingDefineSymbol)
                {
                    scriptingDefineSymbols.RemoveAt(i);
                }
            }
