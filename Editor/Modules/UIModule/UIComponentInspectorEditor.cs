...
namespace Moirai.Atropos.Editor.ModulesUI
{
    ...
    public class UIComponentInspectorEditor : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            ...
            m_genCodePath = serializedObject.FindProperty("m_GenCodePath");
            m_isGenImpClass = serializedObject.FindProperty("m_IsGenImpClass");
            m_impCodePath = serializedObject.FindProperty("m_ImpCodePath");
            m_className = serializedObject.FindProperty("m_ClassName");
            m_uiType = serializedObject.FindProperty("m_UIType");
            ...
            {
                m_genCodePath.stringValue = ScriptGeneratorSettings.GenCodePath;
            }
            if (string.IsNullOrEmpty(m_impCodePath.stringValue))
            {
                m_impCodePath.stringValue = ScriptGeneratorSettings.ImpCodePath;
            }
        ...
        string GetClassName(string targetName)
        {
            ...
            var rules = ScriptGeneratorSettings.ScriptGenerateRules
            ...
            foreach (var rule in rules)
            {
                if (rule.IsUIWidget && targetName.StartsWith(rule.UIElementRegex))
                {
                    ...
                    while (true)
                    {
                        ...
                        if (uiBindComponent)
                        {
                            if (uiBindComponent.m_UIType == ScriptGeneratorSettings.UIGenTypes[0].UITypeName)
                            {
                                targetName = $"{parent.name.Replace("UI", string.Empty)}{targetName.Replace(rule.UIElementRegex, string.Empty)}Widget";
                            }
                            else
                            {
                                targetName = $"{uiBindComponent.m_ClassName.Replace("Widget", string.Empty)}{targetName.Replace(rule.UIElementRegex, string.Empty)}Widget";
                            }
                            break;
                        }
        ...
        private void DrawTopButtons()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("生成UniTask脚本本", GUILayout.Height(25)))
                {
                    // RemoveNullComponents();
            ...
            EditorGUILayout.BeginVertical("HelpBox");
            {
                ...
                m_uiGenTypes = ScriptGeneratorSettings.UIGenTypes;
                // 获取所有的 uiTypeName
        ...
        private static string GetDefaultPathByPropertyType(SerializedProperty property)
        {
            ...
            switch (property.name)
            {
                case "m_GenCodePath":
                case "genCodePath":
                    return ScriptGeneratorSettings.GenCodePath;

                case "m_ImpCodePath":
                case "impCodePath":
                    return ScriptGeneratorSettings.ImpCodePath;
