...
{
    [CustomEditor(typeof(ResourceServiceDriver))]
    internal sealed class ResourceServiceDriverInspector : GameFrameworkInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            ResourceServiceDriver t = (ResourceServiceDriver)target;
        ...
        private void DrawInspectorHeader()
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("资源服务配置", "Resource Service Configuration"),
        ...
        private void DrawBasicSettings(ResourceServiceDriver t)
        {
        ...
        private void DrawResourcePoolSettings(ResourceServiceDriver t)
        {
            ...
            if (m_showResourcePoolSettings)
            {
                EditorGUILayout.BeginVertical("HelpBox");
                {
                    ...
                    bool useSystemUnloadUnusedAssets = EditorGUILayout.ToggleLeft(
                        new GUIContent("使用资源服务卸载回收资源", "启用自动资源回收"),
        ...
        private void DrawDownloadSettings(ResourceServiceDriver t)
        {
            ...
            if (m_showDownloadSettings)
            {
                EditorGUILayout.BeginVertical("HelpBox");
                {
                    ...
                    if (updatableWhilePlaying != m_updatableWhilePlaying.boolValue)
                    {
                        if (EditorApplication.isPlaying)
                        {
                            t.m_UpdatableWhilePlaying = updatableWhilePlaying;
                        }
                        else
                        {
        ...
        private void DrawAdvancedSettings(ResourceServiceDriver t)
        {
            ...
            if (m_showAdvancedSettings)
            {
                EditorGUILayout.BeginVertical("HelpBox");
                {
                    ...
                    if (autoUnloadBundleWhenUnused != m_autoUnloadBundleWhenUnused.boolValue)
                    {
                        if (EditorApplication.isPlaying)
                        {
                            t.m_AutoUnloadBundleWhenUnused = autoUnloadBundleWhenUnused;
                        }
                        else
                        {
        ...
        private void DrawRuntimeInfo(ResourceServiceDriver t)
        {
            if (EditorApplication.isPlaying && IsPrefabInHierarchy(t.gameObject))
            {
        ...
        private void DrawStatistics(ResourceServiceDriver t)
        {
        ...
        private void OnEnable()
        {
            ...
            m_assetPoolCapacity = serializedObject.FindProperty("m_AssetCapacity");
            m_assetExpireTime = serializedObject.FindProperty("m_AssetExpireTime");
            m_assetPoolPriority = serializedObject.FindProperty("m_AssetPriority");
