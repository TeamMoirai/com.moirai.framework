...
namespace Moirai.Atropos.Editor
{
    ...
    /// 打包工具窗口（<see cref="BuildPipelineWindow"/>）与 Project Inspector 共用此绘制。
    /// </summary>
    ...
    internal sealed class BuildConfigEditor : UnityEditor.Editor
    {
        ...
        private void BuildBasicSection(VisualElement parent)
        {
            ...
            _platformPopup = MakePopup("目标平台", s_PlatformNames, name =>
            {
                ...
                if (idx >= 0) { Cfg.BuildTarget = s_PlatformTargets[idx]; Dirty(); }
            });
            ...
            _compressPopup = MakePopup("压缩方式", s_CompressNames, name =>
            {
                ...
                if (idx >= 0) { Cfg.CompressOption = (ECompressOption)idx; Dirty(); }
            });

            var encryptorField = new PropertyField(
                serializedObject.FindProperty("m_EncryptorHandler")) { label = "加密方式" };

            _versionField = MakeTextField("资源版本号", v => { Cfg.PackageVersion = v; Dirty(); });
            var versionRow = MakeRow(_versionField);
            var autoBtn = MakeSmallButton("自动", () =>
            {
                Cfg.PackageVersion = BuildConfig.GetDefaultPackageVersion();
                Dirty();
                _versionField.SetValueWithoutNotify(Cfg.PackageVersion);
            });
            versionRow.Add(autoBtn);

            _outputRootField = MakeTextField("AB输出目录", v => { Cfg.OutputRoot = v; Dirty(); });
            ...
            var browseBtn = MakeSmallButton("浏览", () =>
            {
                ...
                if (!string.IsNullOrEmpty(selected))
                {
                    ...
                    Cfg.OutputRoot = string.IsNullOrEmpty(projectPath) ? selected : projectPath;
                    Dirty();
        ...
        private void BuildAdvancedSection(VisualElement parent)
        {
            ...
            _copyOptionPopup = MakePopup("内置文件拷贝", s_CopyOptionNames, name =>
            {
                ...
                if (idx >= 0) { Cfg.BundledCopyOption = (EBundledCopyOption)idx; Dirty(); }
            });
            _fileNameStylePopup = MakePopup("文件名风格", s_FileNameStyleNames, name =>
            {
                int idx = Array.IndexOf(s_FileNameStyleNames, name);
                if (idx >= 0) { Cfg.FileNameStyle = (EFileNameStyle)idx; Dirty(); }
            });
        ...
        private void BuildPlayerSection(VisualElement parent)
        {
            ...
            {
                int idx = Array.IndexOf(s_PlatformNames, name);
                if (idx >= 0) { Cfg.PlayerPlatform = s_PlatformTargets[idx]; Dirty(); }
            });

            _playerOutputField = MakeTextField("输出路径", v =>
            {
                Cfg.PlayerOutputPath = v;
                Dirty();
            });
        ...
        private void Dirty() => EditorUtility.SetDirty(Cfg);
        ...
        private void RefreshControlsFromConfig()
        {
            ...
            _compressPopup.SetValueWithoutNotify(s_CompressNames[(int)Cfg.CompressOption]);
            _versionField.SetValueWithoutNotify(Cfg.PackageVersion);
            _outputRootField.SetValueWithoutNotify(Cfg.OutputRoot);
            ...
            _copyOptionPopup.SetValueWithoutNotify(s_CopyOptionNames[(int)Cfg.BundledCopyOption]);
            _fileNameStylePopup.SetValueWithoutNotify(s_FileNameStyleNames[(int)Cfg.FileNameStyle]);
