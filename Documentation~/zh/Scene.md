...
> 基于 YooAsset 场景句柄的主/子场景管理模块，提供异步加载、挂起激活、进度回调与子场景卸载能力。

场景模块（`Moirai.Atropos.Scene`）封装 YooAsset 的 `SceneHandle`，区分主场景（`LoadSceneMode.Single`，同时只能存在一个）与子场景（`LoadSceneMode.Additive`，可叠加多个）。支持加载到 90% 时挂起、就绪后统一激活的平滑切换模式，并在主场景加载完成后可选触发资源回收。通过 `GameModule.Scene` 静态访问器使用。
...
| 类/接口 | 说明 |
|---------|------|
| `Moirai.Atropos.Scene.ISceneModule` | 场景模块接口，`GameModule.Scene` 返回此类型 |
| `Moirai.Atropos.Scene.SceneModule` | 场景模块实现，内部持有 `YooAsset.SceneHandle` 管理主/子场景 |
...
GameModule.Scene.Unload("BattleMap", callBack: () => Debug.Log("已卸载"));
...
// 一切就绪后激活场景
bool activated = GameModule.Scene.ActivateScene("GameMain");   // 激活为当前活动场景
bool resumed = GameModule.Scene.UnSuspend("GameMain");         // 仅解除挂起
```
