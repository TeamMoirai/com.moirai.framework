...
- **被污染的时间输入不会破坏轮结构**：tick 换算一律饱和到 `[0, MAX_TICK]`。有限但足以溢出 `double → long` 的延时代谢（如 `1e30f`）按"极远将来"处理——句柄有效、事实上不会触发，也不连累其它计时器；`GameTime.ScaledNow` / `UnscaledNow` 出现 `NaN` 的那一帧整体不推进（游标原地保持），下一帧恢复正常即刻续跑。旧实现里这两类输入会把轮游标打到 `long.MinValue`，在每帧 64 tick 的追赶预算下等价于时间轮永久冻结。
