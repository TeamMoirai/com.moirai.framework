...
通过 `GameApp.ObjectPool` 访问游戏对象池服务。按资源地址管理池化 GameObject 实例，使用编译后的 `PoolCatalog`（ScriptableObject 驱动）解析池规则，支持三种回收策略（Fixed/Burst/Sticky），并提供异步预制体加载与引用计数管理。
...
- **代系校验句柄**：`ObjectPoolHandle`（MonoBehaviour）绑定 slot index + generation，防止 use-after-despawn
...
| 类/接口 | 说明 |
|---------|------|
| `IObjectPoolService` | 服务接口：`Spawn` / `SpawnAsync` / `Despawn` / `Flush` / `FlushGroup` / `FlushAll` / `WarmupAsync` / `LoadCatalog` |
| `ObjectPoolService` | 默认实现（`internal sealed`），`Priority = 6`，实现 `IServiceTickable` 驱动最小堆维护 |
| `RuntimeObjectPool` | 单池运行时：SoA 分页 Slot + 侵入式链表 + generation 句柄 |
| `ObjectPoolHandle` | 附着在池化实例上的 MonoBehaviour；提供 `TryRelease()` 安全回收 |
| `IObjectPoolable` | 池化预制体组件接口：`OnSpawn(in PoolSpawnContext)` / `OnDespawn()` / `OnPooledDestroy()` |
...
| `ObjectPoolSetting` | MonoBehaviour 组件，提供 Inspector 配置 + 低内存/焦点事件注册 |
| `ObjectPoolSnapshot` | 调试快照：每池统计（spawn/despawn/hit/miss/expand/destroy/peak）+ 实例列表 |
...
```csharp
// 或在 ObjectPoolSetting 组件的 Inspector 中指定
...
// 刷新指定分组的所有池
GameApp.ObjectPool.FlushGroup("战斗");
...
- `ObjectPoolHandle` 在实例创建时自动添加。外部 `Destroy()` 池化对象会触发 `NotifyHandleDestroyed` 进行清理。
- 服务由 `IServiceTickable.Tick()` 通过 `GameServices.Tick` 驱动 — 无独立 MonoBehaviour Update 循环。
- `ObjectPoolSetting` 组件在 GameEntry 上提供 Inspector 配置默认值 + 低内存/焦点事件注册（与 `MemoryPoolSetting` 相同模式）。
