...
UI 模块（`Moirai.Atropos.UI`）将界面抽象为纯 C# 类的 `UIWindow` / `UIWidget`，由 `UIModule` 统一管理窗口栈、层级深度与可见性。窗口面板通过资源模块（YooAsset）或 `Resources` 加载实例化，窗口类本身不挂 MonoBehaviour。通过 `GameModule.UI` 静态访问器即可完成打开、关闭、隐藏、查询等全部操作。
...
| 类/接口 | 说明 |
|---------|------|
| `Moirai.Atropos.UI.IUIModule` | UI 模块接口，`GameModule.UI` 返回此类型 |
| `Moirai.Atropos.UI.UIModule` | UI 模块实现，窗口栈管理、深度排序、可见性控制；静态属性 `UIRoot`、`Resource` |
...
| `Moirai.Atropos.UI.UIModuleEvent` | 窗口打开/关闭事件（`Shown` / `Closed`），经 `EventManager` 派发 |
| `Moirai.Atropos.UI.UIModuleHelper` | 交互辅助：`IsInteractionBlockedByModal`、`IsUIObjectInteractable` |
| `Moirai.Atropos.UI.IUIResourceLoader` | UI 资源加载器接口，默认实现 `UIResourceLoader` 走资源模块 |
...
窗口默认内置 0.5 秒打开 / 0.25 秒关闭的等待，可重写替换为动画播放；动画期间窗口自动锁定交互，模态窗口还会联动输入模块（`GameModule.Input.PreventInteractionUI`）：
...
- 全局：静态方法 `UIModule.ApplyScreenSafeRect(Rect)` 直接调整 UIRoot；`UIModule.SimulateIPhoneXNotchScreen()` 在编辑器模拟异形屏。
...
当调试器配置（`DebuggerComp.ActiveWindowType`）判定不启用错误日志时，模块会注册 `ErrorLogger` 捕获 `LogType.Exception`，自动弹出内置 `LogUI` 窗口（`[Window(UILayer.System, fromResources:true)]`，预制体位于模块 `Resources/LogUI.prefab`）逐条查看异常堆栈。
...
- `ShowUI` 同步加载依赖资源模块的同步加载能力，WebGL 下自动退化为异步；建议优先使用 `ShowUIAsync`
