...
出厂的 `StaticSaveKeyProvider` 带着占位口令与盐文（`CHANGE_ME_BEFORE_SHIPPING` / `CHANGE_ME_SALT`），随包发布等于不加密。判据只有一条：`StaticSaveKeyProvider.UsesPlaceholderCredentials` 看**生效值**（运行期覆盖优先于序列化配置）是否为占位或空。Inspector 在处理器上把它标成错误提示；构建期由 `SaveSettingsBuildValidator` 走**同一份判据**（`SaveServiceSettings.UsesPlaceholderSaveKey`）再报一次，默认也只告警——设环境变量 `MOIRAI_SAVE_SETTINGS_STRICT=1` 才会把构建拦停（本包被他人消费，因一项配置拦停别人的构建是工单，不是提醒）。
