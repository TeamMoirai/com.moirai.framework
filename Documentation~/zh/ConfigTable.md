...
`ConfigTable` 模块在 Runtime 侧只包含 `IConfigTableModule` 接口与 `ConfigMgr` 单例：前者约定框架关心的三个能力（多语言文本、图标加载、UI 窗口路径），后者作为全局访问点持有实现。真正的 `ConfigTableModule` 实现、`Tables` 表集合与各表结构均由 Luban 转表工具从模板生成到项目的 `GameProto` 程序集，并通过 `RuntimeInitializeOnLoadMethod` 自动注册到 `ConfigMgr.Instance.ConfigTableModule`。配套的编辑器工具（`Tools/Settings/ConfigTableSettings` 与 `Tools/Config` 菜单）负责配置表工程目录的生成、重定向与转表。

## 核心特性

- 框架与配表解耦：框架仅依赖 `IConfigTableModule` 接口，Luban 生成代码落在业务程序集，移除配表不影响框架其他模块编译
- 懒加载 `Tables`：首次访问 `ConfigTableModule.Tables` 时才加载，按生成代码的 Loader 返回类型自动选择二进制（`ByteBuf`）或 JSON（`JSONNode`）格式
- 编辑器友好：非运行模式下配置 `TextAsset` 直接经 `AssetDatabase` 加载，无需启动资源系统
- 多语言桥接：反射读取生成代码中的 `LocalizedBean` 字段注册可用语言，并将 `TbLocalizedStrings` 展开为 `Dictionary<string, List<string>>` 供 [Localization](Localization.md) 模块使用
...
| 类/接口 | 说明 |
|---------|------|
| `Moirai.Atropos.ConfigTable.IConfigTableModule` | 配置表辅助器接口：`GetAllLocalizedStrings`、`LoadSpriteByID`、`GetUIWindowLocation` |
| `Moirai.Atropos.ConfigTable.ConfigMgr` | `Singleton<ConfigMgr>` 全局入口，持有 `ConfigTableModule` 实例并转发调用 |
| `GameProto.Config.ConfigTableModule` | Luban 生成（模板 `ConfigTableModule.cs` + `ConfigTableModule_Init.cs`），实现 `IConfigTableModule`，含懒加载 `Tables` 与 `Instance` 单例 |
...
// 1. 获取所有多语言文本（Localization 模块启动时调用）
...
- 移动配置表目录后，在设置界面使用「重定向 Config 目录」重新指定；修改导出路径后点击「更新配置路径」，自动同步 `path_export.conf` 各键与 `CustomTemplate/ConfigTableModule_Init.cs` 中的 `CONFIG_PATH` 常量
...
| `ConfigTableModule.cs` | 接口实现：注册、多语言解析、Sprite/UI 查询 |
| `ConfigTableModule_Init.cs` | 加载器：懒加载 `Tables`，桥接 YooAsset 资源系统 |
| `ExternalTypeUtil.cs` | Luban 扩展类型工具 |

## 注意事项

- `ConfigTableModule` 与 `ConfigTableModule_Init` 均为转表生成代码，手动修改会在下次转表时被覆盖；定制逻辑应写在业务侧或修改 `CustomTemplate` 模板
- 配置数据按 PRELOAD 预加载标签打包，运行时 `LoadTextAsset` 走同步 `GameModule.Resource.LoadAsset<TextAsset>`，需确保资源系统已就绪
- 未生成 Config 时 `ConfigMgr.Instance.GetAllLocalizedStrings()` 返回 `null` 并报错 "Generate Config first!"，[Localization](Localization.md) 模块会因此加载失败
