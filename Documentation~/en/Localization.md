...
- Text querying: `GetTextFromId` (supports `string.Format` parameters), `TryGetTextFromId` (single-pass resolve: hit yields the text, miss yields `false`+`null` and feeds the missing-key tracking under the same policy — localizers and inline markers use it for their "inject if present, report if not" check instead of calling `Has` + `GetTextFromId` twice), `GetTextFromIdLanguage` (pass `null` for the current language), `GetDictionaryFromId` (retrieves all languages), `GetAllIds`
