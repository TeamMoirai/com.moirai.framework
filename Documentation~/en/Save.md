...
The shipped `StaticSaveKeyProvider` carries the placeholder passphrase and salt (`CHANGE_ME_BEFORE_SHIPPING` / `CHANGE_ME_SALT`); publishing them unchanged makes the encryption worthless. One rule decides it: `StaticSaveKeyProvider.UsesPlaceholderCredentials` inspects the **effective** values (runtime override takes precedence over the serialized configuration) for placeholder or empty. The Inspector surfaces it as an error box on the handler, and at build time `SaveSettingsBuildValidator` re-runs **the exact same rule** (`SaveServiceSettings.UsesPlaceholderSaveKey`), warning only by default — set `MOIRAI_SAVE_SETTINGS_STRICT=1` to fail the build (this package is consumed by others, so stopping somebody else's build over one setting is a ticket, not a reminder).
