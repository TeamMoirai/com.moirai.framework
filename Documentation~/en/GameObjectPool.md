...
The GameObjectPool service is accessed via `GameApp.GameObjectPool` and manages pooled GameObject instances by resource location. It uses a compiled `PoolCatalog` (ScriptableObject-driven) to resolve pool rules, supports three recycling policies (Fixed/Burst/Sticky), and provides async prefab loading with reference counting.
...
- **Generation-based handle validation**: `GameObjectPoolHandle` (MonoBehaviour) binds each instance with slot index + generation, preventing use-after-despawn
...
|---------|------|
| `IGameObjectPoolService` | Service interface: `Spawn` / `SpawnAsync` / `Despawn` / `Flush` / `FlushGroup` / `FlushAll` / `WarmupAsync` / `LoadCatalog` |
| `GameObjectPoolService` | Default implementation (`internal sealed`), `Priority = 6`, implements `IServiceTickable` for min-heap maintenance |
| `RuntimeGameObjectPool` | Per-location pool: SoA paged slots + intrusive linked list + generation handles |
| `GameObjectPoolHandle` | MonoBehaviour attached to pooled instances; provides `TryRelease()` for safe despawn |
| `IGameObjectPoolable` | Interface for components on pooled prefabs: `OnSpawn(in PoolSpawnContext)` / `OnDespawn()` / `OnPooledDestroy()` |
...
| `GameObjectPoolSetting` | MonoBehaviour component for Inspector-configured pool settings + low memory/focus events |
| `GameObjectPoolSnapshot` | Debug snapshot: per-pool stats (spawn/despawn/hit/miss/expand/destroy/peak) + instance list |
...
```csharp
// Or assign in Inspector on GameObjectPoolSetting component
...
Implement `IGameObjectPoolable` on components attached to pooled prefabs:
...
GameApp.GameObjectPool.FlushGroup("Combat");
...
- `GameObjectPoolHandle` is automatically added to instances on creation. External `Destroy()` of a pooled object triggers `NotifyHandleDestroyed` for cleanup.
- The service is driven by `IServiceTickable.Tick()` via `GameServices.Tick` — no separate MonoBehaviour Update loop.
- `GameObjectPoolSetting` component on GameEntry provides Inspector-configured defaults and low-memory/focus event registration (same pattern as `MemoryPoolSetting`).
