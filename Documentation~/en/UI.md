...
- **`UIService`**: Static facade (`[AutoRegisterService]` + `[HandlerHost(typeof(UIServiceHandler))]` + `[ServiceDependency(typeof(DebuggerService), typeof(ResourceService), typeof(TimerService), typeof(InputService))]`); all public members are static methods/properties that forward through the `Handler` property (fail-fast: lazily initialized when not ready, throws if the default factory is missing, never silently degrades; thread-safe lazy property generated by the source generator)
...
|---------|------|
| `Moirai.Atropos.UI.UIService` | UI service static facade (`[HandlerHost]`); all static APIs for open/close/hide/query; static properties `UIRoot`, `UICamera`, `CurrentModal` |
...
- `GameObject/ScriptGenerator/生成绑定代码` (Generate Binding Code): Generates a `partial class XXX : UIWindow` script and `XXXBinder : UIBindComponent` binding component
