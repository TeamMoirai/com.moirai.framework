...
- **`UIServiceHandler`**: Abstract base class (inherits `FrameworkHandler`) defining the backend contract invoked by the facade; plain runtime class, never serialized
...
- **`UIServiceSettings`**: Framework settings (menu "UI Settings"), selecting the UI backend config (`UIServiceConfig` subclass, handler created via `CreateHandler()` factory) via `[ProviderDropdown]` + `[SerializeReference]`
