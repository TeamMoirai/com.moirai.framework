...
- **`UIService`**: Static facade (`[HandlerHost(typeof(UIHandler))]` + `[ServiceDependency(typeof(ResourceService), typeof(TimerService))]`); all public members are static methods/properties that internally forward to `s_Handler` (thread-safe lazy property generated by the source generator)
- **`UIHandler`**: Serializable backend class (inherits `FrameworkHandler`) carrying the core logic of window stack management, depth sorting, and resource loading; replacing a custom backend requires no changes to callers
- **`UISettings`**: Framework settings (menu "UI Settings"), selecting the UI backend implementation via `[ProviderDropdown]` + `[SerializeReference]`
...
| `Moirai.Atropos.UI.UIHandler` | UI backend handler (inherits `FrameworkHandler`), core logic for window stack management, depth sorting, and visibility control |
| `Moirai.Atropos.UI.UISettings` | Framework settings, selects the UI backend implementation via `[ProviderDropdown]` |
