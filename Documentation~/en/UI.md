...
- **`UIService`**: Static facade (`[HandlerHost(typeof(UIServiceHandler))]` + `[ServiceDependency(typeof(ResourceService), typeof(TimerService))]`); all public members are static methods/properties that internally forward to `s_Handler` (thread-safe lazy property generated by the source generator)
