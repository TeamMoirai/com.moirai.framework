...
- **Addressables degraded queries:** `IsNeedDownloadFromRemote` always returns `false`, `GetAssetInfo` always returns `default`, and the tag-based `GetAssetInfos` always returns an empty array (the matching capability is async-only, or `IResourceLocation` simply carries no tags or size). `HasAsset` reports `AssetOnDisk` for any locatable address and cannot tell "cached" from "needs remote download" (`AssetOnline` never occurs); atlas sub-sprites only support addresses that resolve to a `SpriteAtlas`. Use `YooAssetHandler` in production.
