...
- **Poisoned time input cannot corrupt the wheel**: tick conversion always saturates into `[0, MAX_TICK]`. A finite delay large enough to overflow `double → long` (e.g. `1e30f`) is treated as "very far in the future" — the handle stays valid, it effectively never fires, and it does not affect other timers. A frame whose `GameTime.ScaledNow` / `UnscaledNow` is `NaN` does not advance the wheel at all (the cursor holds its place) and resumes as soon as the clock is sane again. Under the old implementation both inputs drove the wheel cursor to `long.MinValue`, which — with a 64-tick-per-frame catch-up budget — froze the wheel permanently.
