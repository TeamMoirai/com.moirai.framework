...
- `Delay` / `WaitFrame` etc. return `0UL` to signal "not registered": the service is not ready (silent, see above), the arguments are invalid (null callback, `frames <= 0`, `delaySeconds` is `NaN` / `±∞`, or an invalid unsafe binding), or the slot pool is exhausted. The latter two are reported uniformly by both engines via `LogUtility.Warning`, Editor-only — `[Conditional("UNITY_EDITOR")]` strips the call and its argument evaluation from release builds. Valid handles are never 0.
- **Poisoned time input cannot corrupt the wheel**: tick conversion always saturates into `[0, MAX_TICK]`. A finite delay large enough to overflow `double → long` (e.g. `1e30f`) is treated as "very far in the future" — the handle stays valid, it effectively never fires, and it does not affect other timers. When a `GameTime.ScaledNow` / `UnscaledNow` reading is `NaN`, `±∞`, negative, or large enough for the conversion to hit the saturation ceiling, that frame does not advance the wheel at all (the cursor holds its place) and normal operation resumes as soon as the clock is sane; under the same test `Resume` / `Restart` / loop rescheduling are refused outright and the timer keeps its previous state (reported via `LogUtility.Warning` in the Editor). A clock that legitimately moves to a smaller base (handler swap, replay) still pulls the cursor along: the absolute `DueTicks` of already queued timers are unfixable anyway, whereas pinning the cursor in place would leave the queue permanently non-empty and wedge the whole wheel, new registrations included. Under the old implementation `NaN` drove the wheel cursor to `long.MinValue` and infinity to the saturation ceiling, which — with a 64-tick-per-frame catch-up budget — froze the wheel permanently in both cases.
