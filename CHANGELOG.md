...
- 注入器给预览的类型判据收成接缝 `IInjectorAssetPreview`（期望类型名 / 直接接受 / 走自动转换），预览与注入共用加载期那三个判据。
...
- 编辑器预览取资产：`ResourceService.LoadAssetForEditor` 是非播放态唯一的「地址 → 资产」入口（本地化预览、配置表读 `TextAsset` 都走它），直读 `AssetDatabase`、不建租约，后端可覆写 `ResourceServiceHandler.LoadAssetForEditor` 补自己的地址→路径换算。
...
- 非播放态的 Inspector 预览取不到表：预览读的是运行期查询入口，`s_Handler` 未注册时恒为空，等于「不进 Play 就没有预览」；改读 `ConfigTableService.GetAllLocalizedStringsForEditor`，未注册时经 Settings 里配置的处理器取数。
