...
- ⚠ UI 根不再按物体名字查找：改由场景物体上的 `UIRootBinding` 登记（`Awake` 登记、销毁让位、重复登记以后者为准并报错），后端未绑定时报一条 Error 并每帧续等，晚到的加加载入场景也能补上。**迁移**：给原本那个名为 `UIRoot` 的物体加挂 `UIRootBinding` 即可，其上的 Canvas 等配置不动。
