...
- ⚠ UI 根不再按物体名字查找：改由场景物体上的 `UIRootBinding` 登记（`Awake` 登记、销毁让位、重复登记以后者为准并报错，胜者销毁时回补仍在世的最近登记者），后端缺绑定或缺 Canvas 都只报一条问题并每帧续等，晚到的场景/实例化根/事后补上的 Canvas 都补得上。**迁移**：给原本那个名为 `UIRoot` 的物体加挂 `UIRootBinding` 即可，其上的 Canvas 等配置不动。
