...
namespace Moirai.Atropos
{
    ...
    {
        private readonly GameLinkedList<TValue> _linkedList;
        private readonly Dictionary<TKey, GameLinkedListRange<TValue>> _dictionary;
        ...
        {
            _linkedList = new GameLinkedList<TValue>();
        ...
        public void Add(TKey key, TValue value)
        {
            ...
            else
            {
                ...
                _dictionary.Add(key, new GameLinkedListRange<TValue>(first, terminal));
            }
        }
                ...
                for (LinkedListNode<TValue> current = range.First; current != null && current != range.Terminal; current = current.Next)
                {
                    if (current.Value.Equals(value))
                    {
                        if (current == range.First)
                        {
                            ...
                            {
                                _dictionary[key] = new GameLinkedListRange<TValue>(next, range.Terminal);
                            }
                        }
        ...
        {
            private Dictionary<TKey, GameLinkedListRange<TValue>>.Enumerator _enumerator;
            ...
            {
                ((IEnumerator<KeyValuePair<TKey, GameLinkedListRange<TValue>>>)_enumerator).Reset();
            }
        }
    }
}
