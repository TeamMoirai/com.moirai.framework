...
namespace Moirai.Atropos.Save
{
    ...
    public sealed class SaveAssetCatalog : ScriptableObject
    {
        ...
        public sealed class Entry
        {
            ...
            [SerializeField] internal UObject m_Asset;
        ...
        private Dictionary<UObject, string> _assetToLocation;
        ...
        public bool TryGetLocation(UObject asset, out string location)
        {
            if (asset == null)
            {
                location = null;
        ...
        public bool TryResolve<T>(string location, out T asset) where T : UObject
        {
            if (!string.IsNullOrEmpty(location))
            {
                EnsureLookup();
                if (_locationToAsset.TryGetValue(location, out UObject resolved) && resolved != null && resolved is T typed)
                {
                    asset = typed;
                    return true;
                }
        ...
        private void EnsureLookup()
        {
            ...
            _assetToLocation = new Dictionary<UObject, string>(m_Entries.Count);
            _locationToAsset = new Dictionary<string, UObject>(m_Entries.Count, StringComparer.Ordinal);
