...
namespace Moirai.Atropos.Resource
{
    ...
    internal partial class ResourceExtComponent : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField] private int m_MaxProcessPerFrame = 50;
        ...
        private IEnumerator Start()
        {
            ...
            _assetItemPool = GameModule.ObjectPool.CreateMultiSpawnObjectPool<AssetItemObject>(
                "SetAssetPool",
        ...
        private void Update()
        {
            _checkCanReleaseTime += UnityEngine.Time.unscaledDeltaTime;
            if (_checkCanReleaseTime < (double)m_CheckCanReleaseInterval)
            {
                return;
            }
        ...
        /// 回收无引用的缓存资产。
        /// </summary>
        /// <remarks>使用分帧处理优化性能，避免一次性处理大量资源导致卡顿。</remarks>
#if ODIN_INSPECTOR
        ...
        public void ReleaseUnused()
        {
            ...
            // 分帧处理：每帧最多处理 m_MaxProcessPerFrame 个资源
