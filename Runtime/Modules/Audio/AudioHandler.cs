...
{
    /// <summary>
    /// 音频处理器。为游戏提供统一的音效播放接口。
    /// <para>由 <see cref="AudioSettings"/> 序列化配置，可替换为自定义音频后端。</para>
    /// <para>场景3D音效挂到场景物件、技能3D音效挂到技能特效上，并在 <see cref="AudioSource"/> 的Output上设置对应分类的 <see cref="AudioMixerGroup"/>。</para>
    /// </summary>
    [Serializable]
    public class AudioHandler : FrameworkHandler
    {
        /// <summary>
        /// 资源服务引用。由 <see cref="AudioService.OnInit"/> 在构建期注入。
        /// </summary>
        ...
        private Action<AudioHandler, float>[] _trackFadeCallbacks;
        /// <summary>每个音轨一个回调，按 (int)EAudioTrack 索引，惰性初始化</summary>
        private Action<AudioHandler, float>[] TrackFadeCallbacks
        {
            get
            {
                ...
                _trackFadeCallbacks = new Action<AudioHandler, float>[values.Length];
        ...
        private static void OnMasterFadeUpdate(AudioHandler service, float value)
        {
        ...
        public void PutInAudioPool(List<string> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    AssetHandle assetData = ResourceService.LoadAssetAsyncHandle<AudioClip>(path);
