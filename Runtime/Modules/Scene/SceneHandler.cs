...
namespace Moirai.Atropos.Scene
{
    ...
    /// <para>由 <see cref="SceneSettings"/> 序列化配置，可替换为自定义场景加载后端。</para>
    /// </summary>
    [Serializable]
    public class SceneHandler : FrameworkHandler
    {
        /// <summary>
        /// 资源服务引用。由 <see cref="SceneService"/> 在初始化时注入（不参与序列化）。
        /// </summary>
        [NonSerialized] internal ResourceHandler ResourceService;
            ...
            bool gcCollect = true, Action<float> progressCallBack = null)
        {
            ...
            if (sceneMode == LoadSceneMode.Additive)
            {
                ...
                subScene = ResourceService.DefaultPackage.LoadSceneAsync(location, sceneMode, LocalPhysicsMode.None, suspendLoad, priority);
            ...
            else
            {
                ...
                _currentMainScene = ResourceService.DefaultPackage.LoadSceneAsync(location, sceneMode, LocalPhysicsMode.None, suspendLoad, priority);
        ...
        private SceneHandle CreateSceneHandle(string location, string packageName, LoadSceneMode sceneMode, bool suspendLoad, uint priority)
        {
            ...
            {
                return ResourceService.DefaultPackage.LoadSceneAsync(location, sceneMode, LocalPhysicsMode.None, suspendLoad, priority);
            }
