...
namespace Moirai.Atropos.ObjectPool
{
    internal sealed partial class ObjectPoolModule
    {
        ...
        {
            private readonly GameMultiDictionary<string, Object<T>> _objects;
            ...
            public ObjectPool(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority)
                : base(name)
            {
                _objects = new GameMultiDictionary<string, Object<T>>();
            ...
            public override int Capacity
            {
                ...
                set
                {
                    if (value < 0)
                    {
                        throw new GameException("Capacity is invalid.");
                    }
            ...
            public override float ExpireTime
            {
                ...
                set
                {
                    if (value < 0f)
                    {
                        throw new GameException("ExpireTime is invalid.");
                    }
            ...
            public bool CanSpawn(string name)
            {
                if (name == null)
                {
                    throw new GameException("Name is invalid.");
                }
                
                if (_objects.TryGetValue(name, out GameLinkedListRange<Object<T>> objectRange))
                {
            ...
            public void Despawn(object target)
            {
                if (target == null)
                {
                    throw new GameException("Target is invalid.");
                }
            ...
            public void Release(int toReleaseCount, ReleaseObjectFilterCallback<T> releaseObjectFilterCallback)
            {
                ...
                {
                    throw new GameException("Release object filter callback is invalid.");
                }
            ...
            public override ObjectInfo[] GetAllObjectInfos()
            {
                ...
                foreach (KeyValuePair<string, GameLinkedListRange<Object<T>>> objectRanges in _objects)
                {
